 ...
 public class Main
{
	public static void main (String args[])
	{
		...
		while(keepGoing)
		{
			...
			//Making Scope Tree
			System.out.println("Converting from AST to SCT");
			for (int i = 0; i < nodeList1.size(); i++)
			{
				if (nodeList1.get(i).getValue().charAt(0) == '{')
				{
					Node tempScope = new Node("{", givenID);
					givenID++;
					for (int j = i; j < nodeList1.size(); j++)
					{
						tempScope.addChild(nodeList1.get(j));
					}
					nodeList2.add(tempScope);
				}
			...
			{
				System.out.println("(Rule: " + nodeList2.get(i).getValue() + ", ID: " + nodeList2.get(i).getID() + ")");
				ArrayList<Node> childrenTemp = nodeList2.get(i).getChildren();
				if (childrenTemp != null)
				{
					for (int j = 0; j < childrenTemp.size(); j++)
					{	
